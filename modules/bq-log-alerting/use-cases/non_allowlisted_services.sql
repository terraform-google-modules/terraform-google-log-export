...
WITH log_table AS (
  SELECT
    ...
    REGEXP_EXTRACT(authInfo.resource, r '^services/([^/]*)') AS serviceName
  FROM
    `${project}.raw_logs.cloudaudit_googleapis_com_activity_*` log,
...
SELECT
  ...
  callerIp,
  CONCAT('projects/', projectId) AS resourceName,
  principalEmail,
  serviceName
FROM
  log_table
